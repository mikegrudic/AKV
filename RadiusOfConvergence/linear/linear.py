...
def ConformalAKVSol(grid, l, m, coefficient):
    ...
    grid.gthth, grid.gphph = 1+H, (1+H)*np.sin(grid.theta)**2
    grid.UpdateMetric()
    grid.ricci = (2.0 + 2*H**2 + grid.gthth_ph**2/grid.sintheta**2 - grid.D2(H, 1) - grid.costheta/grid.sintheta*grid.gthth_th + grid.gthth_th**2 - grid.D2(H,0) - H*(-4.0 + grid.D2(H,1)/grid.sintheta**2 + grid.costheta/grid.sintheta*grid.gthth_th + grid.D2(H,0)))/(1+H)**3
