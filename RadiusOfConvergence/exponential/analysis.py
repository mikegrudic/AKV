...
fits = np.array([[np.polyfit(a[10:], np.abs(ylm[i,10:,j]),10) for i in xrange(len(lm))] for j in xrange(3)])

eig_fit = np.array([[np.polyfit(a[10:], np.abs(eigs[i,10:,j]),10) for i in xrange(len(lm))] for j in xrange(3)])
